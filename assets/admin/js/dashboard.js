function claculateRetailValue(labels_arr, monthly_value) {
...
//for admin
function claculateRentalTransaction(labels_arr, admin_monthly_value, partner_monthly_value) {
  if ($("#sales-chart").length) {
    ...
    var SalesChart = new Chart(SalesChartCanvas, {
      ...
      data: {
        labels: labels_arr,
        datasets: [{
          label: 'Platform Rental Income',
          data: admin_monthly_value,
          backgroundColor: '#98BDFF'
        },
        {
          label: 'Partner Rental Income',
          data: partner_monthly_value,
...
//for partner
function claculatePartnerRentalTransaction(labels_arr, partner_monthly_value) {
  if ($("#sales-chart").length) {
    var SalesChartCanvas = $("#sales-chart").get(0).getContext("2d");
    var SalesChart = new Chart(SalesChartCanvas, {
      type: 'bar',
...
function CalculateCategoryPie(labels_arr, members, total_members) {
  if ($("#south-america-chart").length) {

    var randomColors = []
    $(labels_arr).each(function (key, val) {
    ...
    var areaOptions = {
      ...
      legendCallback: function (chart) {
        ...
        $(labels_arr).each(function (key, val) {
			...
			var cat_amount = members[key];
			var cat_per = Math.round(cat_amount/total_members*100);
			
          text.push('<div class="d-flex justify-content-between mx-4 mx-xl-5 mt-3"><div class="d-flex align-items-center"><div class="mr-3" style="width:20px; height:20px; border-radius: 50%; background-color: ' + chart.data.datasets[0].backgroundColor[key] + '"></div><p class="mb-0">'+val+' - '+cat_per+'%</p></div>');
          text.push('</div>');
        })
        text.push('</div>');
        text.push('</div>');
    ...
    var southAmericaChartPlugins = {
      beforeDraw: function (chart) {
        ...
        var text = total_members,
		
		
          textX = Math.round((width - ctx.measureText(text).width) / 2),
          textY = height / 2;

        ctx.fillText('€'+formatNumber(text.toString()), textX, textY);
        ctx.save();
      }
    }
...
function CalculateMembershipPie(labels_arr, members, total_members) {
  if ($("#south-america-chart").length) {
    ...
    var areaOptions = {
      ...
      legendCallback: function (chart) {
        ...
        $(labels_arr).each(function (key, val) {
          ...
          text.push('<div class="d-flex justify-content-between mx-4 mx-xl-5 mt-3"><div class="d-flex align-items-center"><div class="mr-3" style="width:20px; height:20px; border-radius: 50%; background-color: ' + chart.data.datasets[0].backgroundColor[key] + '"></div><p class="mb-0">'+val+'</p></div>');
          text.push('</div>');
        })
        text.push('</div>');
        text.push('</div>');
